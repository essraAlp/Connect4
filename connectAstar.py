...
def next_step(window, turn):
    ...
    if other_count == 3 and empty_count == 1: score -= 60
...
# Bu fonksiyon, A* algoritmasını uygular.
# Geçerli sütunları kontrol eder ve her sütun için bir skor hesaplar.
# En yüksek skora sahip sütunu döndürür.
def astar(board, turn):
    ...
    for col in valid_locs:
        ...
        score = toplam_skor(tempboard,col, turn,row)  # Sadece bir adım ileriye bakıyoruz
        if score > best_score:
            best_score = score
            best_col = col
...
# Bu fonksiyon, bir hücre için yol masrafını hesaplar.
# Yatay, dikey ve çapraz yönlerde taşların sayısını kontrol eder.
def hesapla_yol_masrafi(board, row, col, player):
    def kontrol_yon(dx, dy):
        count = 0
        for step in range(1, 4):  # 3 adım kontrol ediliyor
    ...
    # Yatay, dikey, çapraz sol-alt - sağ-üst, çapraz sol-üst - sağ-alt
    yatay = kontrol_yon(0, 1)
    dikey = kontrol_yon(1, 0)
    capraz1 = kontrol_yon(1, 1)
    capraz2 = kontrol_yon(1, -1)

    # Yol masrafı toplamı, her bir yön için ayrı ayrı eklenir.
    toplam_yol_masrafi = (yatay + dikey + capraz1 + capraz2)*10
    return toplam_yol_masrafi

# Bu fonksiyon, toplam skoru hesaplar.
# Normal skor ve yol masrafını birleştirir.
def toplam_skor(board, col, player, row):
    yol_masrafi = hesapla_yol_masrafi(board, row, col, player)
    normal_skor = get_score(board, player)
    return normal_skor + yol_masrafi
