...
# Bu fonksiyon, Minimax algoritmasını uygular.
# Derinlik ve alfa-beta budama kullanarak en iyi hamleyi bulur.
def minimax(board,depth, alpha, beta, maximizingPlayer):
    valid_locs = find_Valid_Columns(board)
    is_finishing = finishing_move(board)

    if depth==0 or is_finishing :
        if is_finishing:
            if winOrNot(board, 1):
                return (None, 100000000000000)
            elif winOrNot(board, 2):
                return (None, -10000000000000)
            else:
                return (None, 0)
        else:
            return (None, get_score(board, 1))
    if maximizingPlayer:
        value = -math.inf
        #if not valid_locs:  # No valid moves left
         #   return (None, get_score(board, 1))
        column = random.choice(valid_locs)
        for col in valid_locs:
            row = find_empty_row(board, col)
            tempboard = board.copy()
            play_move(tempboard, row, col, 1)
            x , score = minimax(tempboard,depth-1, alpha, beta, False)
            if score > value:
                value = score
                column = col
        ...
        for col in valid_locs:
            row = find_empty_row(board, col)
            tempboard = board.copy()
            play_move(tempboard, row, col, 2)
            x , score = minimax(tempboard,depth-1, alpha, beta, True)
            if score < value:
                value = score
                column = col
            beta = min(beta, value)
